...
extension BrochureTreeMixin on BrochureItem {
  ...
  void reassignParent(
      {required List<BrochureItem> root,
      required BrochureItem newParent,
      int? atIndex}) {
    removeFromParent(root);

    if (newParent is BrochureHeader) {
      (atIndex != null)
          ? newParent.items.insert(atIndex, this)
          : newParent.items.add(this);
    } else if (newParent is BrochureSubheader && this is BrochureEntry) {
      (atIndex != null)
          ? newParent.entries.insert(atIndex, this as BrochureEntry)
          : newParent.entries.add(this as BrochureEntry);
    }
  }
...
class BrochureHeader implements BrochureItem {
  ...
  final key = GlobalKey();
  BrochureHeader({this.text = 'Your header here.', List<BrochureItem>? items})
      : items = items ?? [] {
...
class BrochureSubheader implements BrochureItem {
  ...
  final key = GlobalKey();
  BrochureSubheader(
      {this.text = 'Your subheader here.', List<BrochureEntry>? entries})
      : entries = entries ?? [] {
    _controller = TextEditingController(text: text);
  }
  ...
  Widget buildListTile({Widget? leading, Widget? trailing}) {
    return Padding(
        ...
        child: ListTile(
            ...
            contentPadding: const EdgeInsets.all(0),
            title: TextBox(
                controller: _controller, style: const TextStyle(fontSize: 14)),
            leading: leading,
            trailing: trailing));
  }
      ...
      case BrochureHeader _:
        {
          if (entries.isNotEmpty && subheaders.isNotEmpty) {
            brochureMap.insert(0, {
              item.text: [
                {"Entries": List.from(entries)},
                ...List.from(subheaders)
              ]
            });
...
Widget buildBrochureList({
  List<BrochureItem>? brochure,
}) {
  brochure ??= [];
        ...
        footer: (brochureActiveIndex == -1)
            ? Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  IconButton(
                      icon:
                          const Text('+Header', style: TextStyle(fontSize: 12)),
                      onPressed: () {
                        setState(() => brochure!.add(BrochureHeader()));
                      }),
                  IconButton(
                      icon: const Text('+Subheader',
                  ...
                  IconButton(
                      icon:
                          const Text('+Entry', style: TextStyle(fontSize: 12)),
                      onPressed: () {
                          ...
                          brochure![index].buildListTile(
                            ...
                            trailing: (brochureActiveIndex == index)
                                ? IconButton(
                                    ...
                                    onPressed: () => setState(
                                        () => brochure!.removeAt(index)),
                                  )
                                : null,
                          ),
                          Row(mainAxisSize: MainAxisSize.min, children: [
                            IconButton(
                                icon: const Text('+Header',
                                    style: TextStyle(fontSize: 12)),
                                onPressed: () {
                                  setState(() => brochure!
                                      .insert(index + 1, BrochureHeader()));
                                }),
                            IconButton(
                                icon: const Text('+Subheader',
                                    style: TextStyle(fontSize: 12)),
                                onPressed: () {
                                  setState(() => brochure!
                                      .insert(index + 1, BrochureSubheader()));
                                }),
                            IconButton(
                                icon: const Text('+Entry',
                                    style: TextStyle(fontSize: 12)),
                                onPressed: () {
                                  setState(() => brochure!
                                      .insert(index + 1, BrochureEntry()));
                                })
                          ])
                        ])
                  : brochure![index].buildListTile(
                      leading: ReorderableDragStartListener(
                        index: index,
                        child:
                            const Icon(FluentIcons.chevron_up_down_20_regular),
                      ),
                      trailing: (brochureActiveIndex == index)
                          ? IconButton(
                              icon: const Icon(FluentIcons.delete_20_regular),
                              onPressed: () =>
                                  setState(() => brochure!.removeAt(index)),
                            )
                          : null,
                    ));
        },
      ...
      return Column(children: [
        ...
        Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              IconButton(
                  icon:
                      const Text('+Subheader', style: TextStyle(fontSize: 12)),
                  onPressed: () {
                    setState(() => brochure!.add(BrochureSubheader()));
                  }),
              IconButton(
      ...
      return TreeDragTarget<BrochureItem>(
        ...
        onNodeAccepted: (TreeDragAndDropDetails details) {
          ...
          if (targetParent == draggedNode.getParent(brochure)) {
            details.mapDropPosition(
              ...
              whenInside: () {
                if (targetNode is BrochureHeader ||
                    targetNode is BrochureSubheader) {
                  (draggedNode)
                      .reassignParent(root: brochure!, newParent: targetNode);
                }
              },
              whenBelow: () {
                if (entry.isExpanded) {
                  if (targetNode is BrochureHeader ||
                    ...
                    draggedNode.reassignParent(
                        root: brochure!, newParent: targetNode, atIndex: 0);
                  }
                } else {
                  int targetIndex = 0;
                  if (targetParent != null) {
                    targetIndex =
                        targetParent.getChildren().indexOf(targetNode);
                  } else {
                    targetIndex = brochure!.indexOf(targetNode);
                  }
                  draggedNode.reorderItem(
                      root: brochure!, newIndex: targetIndex + 1);
                }
              },
            );
          } else {
            details.mapDropPosition(
              whenAbove: () {
                ...
                (draggedNode).reassignParent(
                    root: brochure!, newParent: targetNode, atIndex: newIndex);
              },
              whenInside: () {
                if (targetNode is BrochureHeader ||
                    targetNode is BrochureSubheader) {
        ...
        builder: (BuildContext context, TreeDragAndDropDetails? details) {
          ...
          if (details != null) {
            ...
            decoration = BoxDecoration(
              border: details.mapDropPosition(
                ...
                whenInside: () => (entry.node is BrochureHeader ||
                        entry.node is BrochureSubheader)
                    ? const Border.fromBorderSide(borderSide)
                    : null,
                whenBelow: () =>
                    entry.isExpanded ? null : const Border(bottom: borderSide),
              ),
            );
          }
          ...
          return TreeIndentation(
              guide:
                  const IndentGuide.connectingLines(thickness: 2, indent: 50),
              entry: entry,
