        ...
        body: GestureDetector(
            ...
            child: Center(
              ...
              Align(
                  ...
                  child: Text(
                      '${AppInfo.packageInfo.version} ${AppInfo.sessionId}'))
            ]))));
  }
    ...
    _timer = Timer(const Duration(minutes: 10), () {
      if (mounted) {
        Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (context) => const IdlePage()));
                    ...
                    child: FadeTransition(
                        ...
                        child: GridView.builder(
                          padding: const EdgeInsets.only(
                              top: 110, bottom: 20, left: 20, right: 20),
                          ...
                          itemBuilder: (context, index) => ProductManager
                              .all!.catalogItems[index]
                              .buildCard(() => catalogNavigation(
                                  context,
                                  ProductManager.all!
                                      .getAllCatalogItems()[index])),
                        )),
                  )
                ], //)
              ),
                  ...
                  GestureDetector(
                    onDoubleTap: () {
                      ...
                      Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                              builder: (context) => const IdlePage()));
                    },
                    child: Padding(
                        ...
                        child: Hero(
                            tag: 'main-logo',
                            child: Image.asset(
                              'assets/weightech_logo_beta.png',
                              height: 100,
...
class ProductPage extends StatefulWidget {
  const ProductPage(
      {super.key, required this.product, this.animateDivider = false});
  ...
  void initState() {
    ...
    _fadeAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
        CurvedAnimation(
            ...
            curve: Interval(widget.animateDivider ? 0.3 : 0.1,
                widget.animateDivider ? 0.6 : 0.4,
                curve: Curves.ease)));
    _dividerHeightAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
        CurvedAnimation(
            parent: _animationController,
    ...
    _timer = Timer(const Duration(minutes: 10), () {
      if (mounted) {
        Navigator.pushReplacement(
            context, MaterialPageRoute(builder: (context) => const IdlePage()));
        Log.logger.t("--Idle Timeout--");
      }
    });
            ...
            children: [
              widget.product.description?.isNotEmpty ?? false
                  ? FadeTransition(
                      ...
                      child: Align(
                          alignment: Alignment.center,
                          child: SimpleRichText(widget.product.description!,
              ...
              FadeTransition(
                ...
                child: Column(children: [
                  CarouselSlider.builder(
                      options: CarouselOptions(
                        enableInfiniteScroll:
                            widget.product.productMedia!.length > 1
                                ? true
                                : false,
                      ...
                      itemBuilder: (BuildContext context, int itemIndex,
                        ...
                        return FutureBuilder(
                            future: FileUtils.cacheManager.getSingleFile(widget
                                .product
                                .productMedia![itemIndex]['downloadUrl']),
                            builder: ((context, snapshot) {
                              if (snapshot.hasData) {
                                if (p.extension(snapshot.data!.path) ==
                                    '.mp4') {
                  ...
                  if (widget.product.productMedia!.length > 1)
                    Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: widget.product.productMedia!
                          .asMap()
                          .entries
                          .map((entry) {
                        return Container(
            ...
            child: Padding(
                ...
                child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      final headerKey =
                          widget.product.brochure![index].keys.first;
                      final headerValue =
                          widget.product.brochure![index][headerKey] as List;
                      final headerEntries = headerValue.singleWhere(
                          (element) => (element as Map).keys.first == "Entries",
                          orElse: () => <String, List<String>>{})["Entries"];
                      final subheaders = List.from(headerValue);
                      subheaders.removeWhere(
                          (element) => element.keys.first == "Entries");
                          ...
                          children: [
                            ...
                            ListView.builder(
                              ...
                              itemBuilder: (context, subIndex) {
                                final subheaderKey =
                                    subheaders[subIndex].keys.first;
                                final subheaderValue = subheaders[subIndex]
                                    [subheaderKey] as List<dynamic>;
                  ...
                  child: Column(children: [
                    CarouselSlider.builder(
                        ...
                        itemBuilder: (BuildContext context, int itemIndex,
                            int pageViewIndex) {
                          return FutureBuilder(
                              future: FileUtils.cacheManager.getSingleFile(
                                  widget.product.productMedia![itemIndex]
                                      ['downloadUrl']),
                              builder: ((context, snapshot) {
                                if (snapshot.hasData) {
                                  if (p.extension(snapshot.data!.path) ==
                ...
                child: Padding(
                    ...
                    child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final headerKey =
                              widget.product.brochure![index].keys.first;
                          final headerValue = widget.product.brochure![index]
                              [headerKey] as List;
                          final headerEntries = headerValue.singleWhere(
                              (element) =>
                                  (element as Map).keys.first == "Entries",
                              orElse: () =>
                                  <String, List<String>>{})["Entries"];
                          final subheaders = List.from(headerValue);
                          subheaders.removeWhere(
                              (element) => element.keys.first == "Entries");
...
class ListingPage extends StatefulWidget {
  ListingPage({super.key, required this.category, this.animateDivider = false})
  ...
  void catalogNavigation(BuildContext context, dynamic item) {
    if (item is ProductCategory) {
      ...
      Navigator.push(
          ...
          PageRouteBuilder(
              pageBuilder: (context, animation, secondaryAnimation) =>
                  ListingPage(category: item, animateDivider: false),
              transitionsBuilder:
                  (context, animation, secondaryAnimation, child) {
                var begin = 0.0;
                var end = 1.0;
                ...
                children: <Widget>[
                  Flexible(
                      child: FadeTransition(
                          ...
                          child: GridView.builder(
                            padding: const EdgeInsets.only(
                                top: 165, bottom: 20, left: 20, right: 20),
                            ...
                            itemBuilder: (context, index) => widget
                                .catalogItems[index]
                                .buildCard(() => catalogNavigation(
                                    context, widget.catalogItems[index])),
                          )))
                ],
              ),
              Stack(alignment: Alignment.topCenter, children: [
